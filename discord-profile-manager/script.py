...
class Games:
	...
	def add(self):
		if self.database.REPLACEME.games.count_documents({'id' : self.id}) > 0:
			self.database.REPLACEME.games.update_one({'id' : self.id}, {'$push' : {'games' : self.game}})
		else:
			self.database.REPLACEME.games.insert_one({'id' : self.id, 'games' : [self.game]})

	def remove(self):
		if self.database.REPLACEME.games.count_documents({'id' : self.id}) > 0:
			self.database.REPLACEME.games.update_one({'id' : self.id}, {'$pull' : {'games' : self.game}})
...
class Profile:
	...
	def add(self):
		if self.database.REPLACEME.profiles.count_documents({'id' : self.id}) > 0:
			self.database.REPLACEME.profiles.update_one({'id' : self.id}, {'$push' : {'usernames' : '{}:{}'.format(self.social, self.username)}})
		else:
			self.database.REPLACEME.profiles.insert_one({'id' : self.id, 'usernames' : ['{}:{}'.format(self.social, self.username)]})

	def remove(self):
		if self.database.REPLACEME.profiles.count_documents({'id' : self.id}) > 0:
			self.database.REPLACEME.profiles.update_one({'id' : self.id}, {'$pull' : {'usernames' : '{}:{}'.format(self.social, self.username)}})
...
async def games(ctx, member: discord.User=None):
	if is_channel(ctx.channel.name, 'use-commands'):
		dict = database.REPLACEME.games.find_one({'id' : member.id if member != None else ctx.message.author.id})
...
async def socials(ctx, member: discord.User=None):
	if is_channel(ctx.channel.name, 'use-commands'):
		usernames = database.REPLACEME.profiles.find_one({'id' : member.id if member != None else ctx.message.author.id})['usernames']
...
async def create_game_code(ctx, code, name):
	if is_channel(ctx.channel.name, 'manage-games'):
		database.REPLACEME.game_ids.insert_one({'code' : code, 'name' : name}) if database.REPLACEME.game_ids.count_documents({'code' : code}) == 0 else await ctx.channel.send('{} has already been added'.format(code))
		await ctx.message.delete()

@bot.command()
async def create_social_code(ctx, code, name):
	if is_channel(ctx.channel.name, 'manage-socials'):
		database.REPLACEME.platform_ids.insert_one({'code' : code, 'name' : name}) if database.REPLACEME.social_ids.count_documents({'code' : code}) == 0 else await ctx.channel.send('{} has already been added'.format(code))
		await ctx.message.delete()
...
def get_all_game_codes():
	ids = []
	game_ids = database.REPLACEME.game_ids.find({})
...
def get_all_social_codes():
	ids = []
	social_ids = database.REPLACEME.platform_ids.find({})
...
def get_name_from_code(code):
	return database.REPLACEME.game_ids.find_one({'code' : code})['name']

def get_social_name_from_code(code):
	return database.REPLACEME.platform_ids.find_one({'code' : code})['name']
